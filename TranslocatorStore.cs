...
public class TranslocatorStore
{
    ...
    // AND once per dropdown label on every rebuild.
    ...
    /// (cheap), but the file write runs on the thread pool unless
    ...
    public bool AddDiscovered(BlockPos src, BlockPos dst, long chunkIdx)
    {
        lock (_lock)
        {
            ...
            if (_entries.TryGetValue(key, out var ex))
            {
                ...
                ex.Dst = dst;
                _version++; // Dst may have changed; snapshots must refresh
                return false;
            }
    ...
    private int IngestSaveFileLocked(TlSaveFile f, string fallbackOwner)
    {
        ...
        // but cached snapshots and counts still need to see the new entries.
        if (added > 0)
        {
            _countsByGroup = null;
            _version++;
